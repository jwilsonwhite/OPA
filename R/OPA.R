...
#' function such as \code{\link[spatstat]{spatstat::plot.ppp}}, \code{\link[spatstat]{spatstat::plot.fv}}  and \code{\link[spatstat]{spatstat::envelope}} can be used to process
...
#' @examples 
#' \donttest{
#' X = random.landscape(maxR = 100,n=50,range.pts = 10,range.radii = 1,radius.mean=1,radius.sd=1) # A 100 m x 100 m landscape of objects that are spatially clustered at a range of 10 m
#' K = K.obj(X,Type='rect')
#' plot(K) # plot the K function versus distance (r)
#' plot(K,L~r) # plot the scaled L statistic instead (expectation = 1 under complete spatial randomness)
#' 
#' # Use spatstat::envelope to calculate a significance band around the K statistic (note that this is computationally intensive and may take a long time!):
#' KE = envelope(X,fun=K.obj,funargs=list(Type='rect'),
#' nsim=1e3, simulate=expression(random.landscape(100,50,range.pts=1,range.radii=1,radius.mean=1,radius.sd=1)))
#' 
#' # Use spatstat::envelope to calculate a significance band around the L statistic, which is a scaled transformation of the L statistic. To perform this scaling you must directly specify the \code{Eps} parameter:
#' Eps = 0.01
#' KE = envelope(X,fun=K.obj,funargs=list(Type='rect',Eps=Eps),
#' nsim=1e3, simulate=expression(random.landscape(100,50,range.pts=1,range.radii=1,radius.mean=1,radius.sd=1)),transform = expression(./(2*pi*r*Eps)))
...
#' nsim=1e3, simulate=expression(random.landscape(units=units,n=n,radius.mean=rm,radius.sd=rs,Type='circle')),transform = expression(./(2*pi*r*Eps)))
